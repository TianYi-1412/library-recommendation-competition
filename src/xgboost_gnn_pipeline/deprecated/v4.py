...
from gnn_feature import GNNEmbedder  # <-- 新增

warnings.filterwarnings('ignore')


class PureNewBreakthroughRecommenderV4:
    def __init__(self):
        # 原有数据结构
        ...
        # 时序特征
        self.user_borrow_interval_mean = {}
        self.user_borrow_dow_pref = {}
        self.user_borrow_month_pref = {}
        self.user_last_borrow_cate = {}
        ...
        self.candidate_layers = {'weak_feature': 0.9, 'hot': 0.1}
    ...
    def load_data(self, inter_path, book_path, user_path):
        ...
        self._compute_user_temporal_features()
    ...
    # -------------------- 时序特征 --------------------
    def _compute_user_temporal_features(self):
        for user_id, records in self.user_train_data.items():
            borrow_times = [r['借阅时间'] for r in records]
            borrow_times.sort()
            intervals = [(borrow_times[i] - borrow_times[i-1]).days for i in range(1, len(borrow_times))]
            self.user_borrow_interval_mean[user_id] = np.mean(intervals) if intervals else 30
            self.user_borrow_dow_pref[user_id] = Counter([t.weekday() for t in borrow_times]).most_common(1)[0][0]
            self.user_borrow_month_pref[user_id] = Counter([t.month for t in borrow_times]).most_common(1)[0][0]
            self.user_last_borrow_cate[user_id] = self.book_info.get(records[-1]['book_id'], {}).get('二级分类', '未知')

    # -------------------- 负采样 --------------------
    def _sample_negatives_fast(self, user_id, n=3):
        all_books = np.array(list(self.book_info.keys()))
        borrowed = self.user_borrow_history[user_id]
        mask = ~np.isin(all_books, list(borrowed))
        neg_candidates = all_books[mask]
        if len(neg_candidates) == 0:
            return []
        return np.random.choice(neg_candidates, size=min(n, len(neg_candidates)), replace=False)
    ...
    def build_train_features(self):
        ...
        features, labels = [], []
        for user_id, records in self.user_train_data.items():
            pos_books = [r['book_id'] for r in records]
            neg_books = self._sample_negatives_fast(user_id, n=min(3, len(pos_books)))
            for book_id in pos_books + list(neg_books):
                label = 1 if book_id in pos_books else 0
                feat = self._extract_feature_vector(user_id, book_id)
                features.append(feat)
                labels.append(label)
        sample_weights = compute_sample_weight(class_weight={0: 0.3, 1: 8}, y=labels)
    ...
    def _extract_feature_vector(self, user_id, book_id):
        ...
        return [renew_score, hot_score, secondary_match, dept_match, keyword_match,
                dept_secondary_cross, keyword_secondary_cross, gnn_sim]
    ...
    def recommend_for_user(self, user_id):
        ...
        weak_candidates = self._get_weak_candidates(user_id)
        hot_candidates = self._get_hot_candidates(user_id)
        final_candidates = weak_candidates[:45] + hot_candidates[:5]
        if not final_candidates:
            return None

        features = [self._extract_feature_vector(user_id, b) for b in final_candidates]
        scores = self.model.predict_proba(features)[:, 1]
        return final_candidates[np.argmax(scores)]

    def _get_weak_candidates(self, user_id):
        ...
        hot_threshold = np.percentile(list(self.book_hot.values()), self.high_hot_threshold)
        candidates = [
            b for b in candidates
            if (user_id, b) not in self.user_book_renew
            and self.book_hot.get(b, 0) < hot_threshold
            and b not in self.user_borrow_history[user_id]
        ]
        return candidates

    def _get_hot_candidates(self, user_id):
        hot_books = sorted(self.book_hot.items(), key=lambda x: x[1], reverse=True)[:50]
        return [b for b, _ in hot_books if b not in self.user_borrow_history[user_id]]
    ...
    def save_recommendations(self):
        ...
        for user_id in self.user_val_data.keys():
            pred_book = self.recommend_for_user(user_id)
            true_book = self.user_val_data[user_id]
        ...
        df.to_csv('v4_recommendations.csv', index=False, encoding='utf-8-sig')
        print(f"✅ 推荐结果已保存为 v4_recommendations.csv，耗时：{time.time() - start:.2f}秒")
    ...
    def main(cls, inter_path, book_path, user_path):
        ...
        f1 = rec.evaluate()
        rec.save_recommendations()  # ← 新增
        print(f"总耗时：{time.time() - start:.2f}秒")
        return f1


if __name__ == "__main__":
    PureNewBreakthroughRecommenderV4.main(
        inter_path='./datasets/pre_inter.csv',
        book_path='./datasets/book_data.csv',
